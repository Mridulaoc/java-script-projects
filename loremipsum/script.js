...
const generatePara = (e) => {
  e.preventDefault();
  if (isNaN(paraCount.value) || (paraCount.value) < 0 || (paraCount.value) > 9) {
    const randomIndex = Math.floor(Math.random() * text.length);
    paraContainer.innerHTML=`<p>${text[randomIndex]}</p>`;
  }
  else {
    const data = text.slice(0, paraCount.value);
    const paras = data.map((data)=>{
      return `<p>${data}</p>`;
    })
